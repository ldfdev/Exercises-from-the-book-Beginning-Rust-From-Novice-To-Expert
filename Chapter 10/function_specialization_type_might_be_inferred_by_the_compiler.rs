...
fn main() {
    ...
    //below the function specialization type is inferred by the compiler
    //  otherwise the call should have been written as
    //  pick_a_nice_element::<i64>(&some_elements)
    let picked = pick_a_nice_element(&some_elements);
    println!("{}", picked)
}